...
class Angles{
  ...
  async getPoses() {
    ...
    let lx_hip;
    let ly_hip;
    poses = await detector.estimatePoses(this.video.elt);
    ...
    console.log("turn angle 1")
    
    setTimeout(self, 0);
    console.log("turn angle 2")
  }
  ...
  get turnAngle() {
    return this.getPoses();
  }
    // check that both shoulder angles are above, say 30 deg - do this in main, not here
  get leftAngle() {
    return this.rightShoulderAngle();    
  }
  
  get rightAngle() {
    return this.leftShoulderAngle();
  }
}
